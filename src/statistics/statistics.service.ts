...
export class StatisticsService {
  ...
  async getMainStatistics(storeId: string) {
    ...
    const categoriesCount = await this.countCategorirs(storeId);
  ...
  async getMiddleStatistics(storeId: string) {
    ...
    const lastUser = await this.getLastUsers(storeId);

    return { monthlySales, lastUser };
  }
  ...
  // Количество продуктов у магазина
  ...
  // Количество категорий у магазина
  private async countCategorirs(storeId: string) {
  ...
  // Получение средннего рейтинга товаров для магазина
  ...
  // Последние 5 пользователей совершившие заказ
